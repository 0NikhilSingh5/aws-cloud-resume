...
export function AnimatedThemeToggler({
  ...
  const toggle = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // Capture click position so the reveal can expand from the toggle button
    const root = document.documentElement;
    root.style.setProperty("--tt-x", `${e.clientX}px`);
    root.style.setProperty("--tt-y", `${e.clientY}px`);
    ...
    // Use the View Transitions API where supported; fall back to instant flip
    type RootWithVT = Document & {
      startViewTransition?: (cb: () => void) => unknown;
    };
    const docVT = document as RootWithVT;
    if (typeof docVT.startViewTransition === "function") {
      docVT.startViewTransition(flip);
    } else {
      flip();
    }
  };
